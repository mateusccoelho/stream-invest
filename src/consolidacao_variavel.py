...
def calcula_qtd_preco_medio(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for qtd_sinal, valor_trans in zip(df["qtd_sinal"], df["valor_trans"]):
        ...
        if qtd_sinal > 0:
            preco_medio.append((preco_medio[-1] * qtd[-1] + valor_trans) / nova_qtd)
        else:
            preco_medio.append(preco_medio[-1])
        qtd.append(nova_qtd)

    return pd.DataFrame({"qtd_acum": qtd[1:], "preco_medio": preco_medio[1:]})
...
def consolidar_renda_variavel(
    ...
    # "atravessa" os ativos.
    patrimonio_rv = dias.merge(preco_medio, how="left", on=["codigo", "data"]).merge(
        cotacoes, how="left", on=["codigo", "data"]
    )
    ...
    carteira_rv = patrimonio_rv.groupby("codigo").last().assign(status=1)
